        ...
        SELECT
          ...
          IFNULL(
            ...
            FROM
              sim.OCEK 
            WHERE
              employee_id = T0.employee_id 
              AND LEFT ( document_date, 10 ) = T0.document_date 
              AND T0.id_ocst = id_ocst 
...
export const removeTimeSheet = async (req, res) => {
  try {
    await OTMS.update(
      { is_active: "0" },
      {
        where: {
          id_otms: req.body.id_otms,
        },
      }
    );
